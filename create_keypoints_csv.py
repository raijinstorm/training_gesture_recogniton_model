...
def get_label_from_filename(filename, gestures):
    """
    Extracts the gesture label from the beginning of the filename.
...
def main():
    ...
    parser.add_argument('--folder', required=True, help='images2')
    ...
    for image_path in image_paths:
        # Get the gesture label from the filename.
        label = get_label_from_filename(image_path, gestures)
        if label is None:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                logging_csv(label, pre_processed_landmarks)
                print(f"Processed {image_path} with label {label}")
        else:
